...
TEST(HW1Test, TEST2) {
  using namespace book;
  EXPECT_THROW(read_database("inventory_wrong_filename.txt"),
               std::runtime_error);
}
...
TEST(HW1Test, TEST10) {
  ...
  order(inventory, shopinglist,
        search(inventory,
               "The Lord of the Rings:The Art of the Fellowship of the Ring"));
  order(inventory, shopinglist,
        search(inventory, "Neither Here nor There: Travels in Europe"));

  EXPECT_EQ(shopinglist.size(), 3);
}
...
TEST(HW1Test, TEST11) {
  ...
  order(inventory, shopinglist,
        search(inventory, "What to Expect the First Year FAKE"));
  order(inventory, shopinglist, search(inventory, "Heidi"));
  order(inventory, shopinglist, search(inventory, "Giving Good Weight"));
  order(inventory, shopinglist,
        search(inventory, "The Control of Nature FAKE"));
  order(inventory, shopinglist,
        search(inventory,
               "The Lord of the Rings: The Art of the Fellowship of the Ring"));
  order(inventory, shopinglist,
